...
function prepareInternalnav() {
  if (!document.getElementsByTagName) return false;
  if (!document.getElementById) return false;
  var links = document.getElementsByTagName("article")[0].getElementsByTagName("a");
  if (links.length == 0) return false;
  for (var i = 0; i < links.length; i++) {
    var current_id = links[i].href.split("#")[1];
    if (!document.getElementById(current_id)) continue;
    document.getElementById(current_id).style.display = "none";
    links[i].destination = current_id;
...
function showPic(whichpic) {
  ...
  description.firstChild.nodeValue = whichpic.title ? whichpic.title : "";
}
